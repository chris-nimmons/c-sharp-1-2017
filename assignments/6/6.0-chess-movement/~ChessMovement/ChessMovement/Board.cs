...
namespace ChessMovement
{
    public class Board
    {
        ...
        public void PieceBoardRepopulator()
        {
            foreach (var piece in Pieces)
            {
                if (!(piece.Taken))
                {
                    piece.Board = new Cell[8, 8];
                            ...
                            else if (selection != null)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (moves.Exists(m => m.X == relativeX && m.Y == relativeY))
                                    {
                                        ...
                                        {
                                            var goner = selection.Board[relativeX, relativeY].Piece;

                                            foreach (var piece in Pieces)
                                            {
                                                if (piece.ID == goner.ID)
                                                {
                                                    piece.Taken = true;
                                                    break;
                                                }
